...
int desiredBrightness = 700; // desired brightness (set by user)
...
int StartUnbrighten(int inside, int outside) {
  ...
  spinningCounterclockwise = BlindAdjustmentCounterclockwise(outside); //Adjust the blinds and keep track of how much time they spun clockwise
  if (desiredBrightness < inside + 80)
  {
    LightDecrease(); //Adjust Light if desired threshold has not been reached
  }
...
int DecreaseLight() {
  int sensorValue  = analogRead (A2);
  int insideLight = sensorValue;
  int brightnessDecrease = -1; //Increase brightness by 1 every iteration
  while (insideLight  > desiredBrightness) {
    sensorValue  = analogRead (A2);
    insideLight = sensorValue;
    // set the the brightness:
